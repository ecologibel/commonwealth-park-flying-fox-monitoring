...
Here we present the abundance and density results generated as part of the ABS' annual report to the NCA. Note that the reporting period for each year extends Dec 1–Nov 30, and the species surveyed included **grey-headed flying-foxes** (GHFFs) and **little red flying-foxes** (LRFFs).
...
#ggmap::register_google(key="enter_your_API_key_here", write=TRUE)
...
occ <- read_excel(raw_data, sheet="occupation") %>%
  ...
  mutate(across(c(peak_date, departure, arrival, 
         ...
         arr_date=as.Date(paste("2012", format(arrival, 
                                        "%m-%d"), sep="-")),
         dep_date=as.Date(paste("2012", format(departure, 
                                        "%m-%d"), sep="-")),
         peak_date=as.Date(paste("2012", format(peak_date, 
                                         "%m-%d"), sep="-")),
...
ggsave(plot=occ_plot, 
       ...
       width=6000, height=3500, units="px", dpi=800)
```
