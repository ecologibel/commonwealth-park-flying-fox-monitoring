...
This [R project](https://support.posit.co/hc/en-us/articles/200526207-Using-RStudio-Projects) presents the datasets and workflows used in the '[ABS Commonwealth Park flying-fox monitoring program](https://sites.google.com/view/absflyingfoxmonitoringact/) publication *Hanging in there: the highs and lows of a flying-fox camp in a novel landscape*'. For information, visit the [GitHub repo](https://github.com/ecologibel/commonwealth-park-flying-fox-monitoring). 
...
> Run this chunk once when setting up a new project. 

```{r, eval = FALSE}
# Install ccl package from GitHub (run once; skip if already installed)
remotes::install_github("coexistence-conservation-lab/ccl")
```

> Run this chunk every time you load the project. 

```{r, message = FALSE, results = 'hide', warning = FALSE}
# Update ccl package if a newer version is available
remotes::update_packages("ccl")
...
pacman::p_load(
  ...
  # Data wrangling 
  broom, ggh4x, grid, hms, janitor, tidyverse, naniar, reshape2, 
...
Prints a formatted table of abbreviations and key terms. 
...
```{r}
# Check and register Google Maps API key in metadata folder
check_google_maps_api_key()
```
...
REP_START     <- "2024-12-01"
REP_END       <- "2025-11-30"
REP_YEAR          <- 2025

# Define dates for tree-by-tree surveys
TREE_START    <- as.Date("2018-12-15")
TREE_END      <- as.Date("2024-09-29")

# Define dates for all surveys
ALL_START     <- as.Date("2012-01-01")
ALL_END       <- REP_END
...
dat_cur <- dat_wide %>%
  filter(date > REP_START & date < REP_END) %>%
  
  # Include LRFFs if LRFFS_PRESENT = TRUE 
  filter(LRFFS_PRESENT | species != "lrff")
...
dat_all <- dat %>%
  ...
  filter(year >= lubridate::year(as.Date(ALL_START))) %>% 
...
quiet((survey_weather_plot <- ggplot() +
  ...
  geom_rect(
    aes(
      xmax      = as.Date(REP_END),
      xmin      = as.Date(ALL_START),
      ymax      = (max(enso$nino_3_4) * 800) - ENSO_ADJ,
      ymin      = (ENSO_NEU           * 800) - ENSO_ADJ),
    alpha       = 0.12,
    fill        = vir1, 
    inherit.aes = FALSE) +  
...
data <- surveys_trees %>%
  ...
  subset(date > REP_START & date < REP_END) %>%
...
Since 2019, we estimated the range (minimum and maximum) proportion of canopy cover for the four main tree types at the Commonweath Park FF camp: 
  ...
  - **Broadleaves:** (e.g., elm *Ulmus* spp., oak *Quercus* spp., poplar *Populus* spp., and silver birch *Betula pendula*), 
  - **Needleleaves:** (e.g., cedar *Cedrus* spp., cypress *Cupressus* spp., sequoia *Sequoiadendron* spp.), 
  - **Eucalypts:** (e.g., Blakely's red gum *Eucalyptus blakelyi*, mealy bundy *Eucalyptus nortonii*, river peppermint *Eucalyptus elata*), and 
...
# Prepare minimum and maximum temperature data
weather_can <- weather %>%
  mutate(
    date      = as.Date(date), 
    max_temp  = as.numeric(max), 
    min_temp  = as.numeric(min), 
    mean_temp = (as.numeric(min) + as.numeric(max)) / 2) %>%
  select(date, max_temp, min_temp, mean_temp) 

# Prepare canopy cover measures
canopy <- read_excel(raw_data, sheet = "surveys", na = c("N/A")) %>%
  ...
  # Parse only numeric canopy cover values
  mutate(canopy = readr::parse_number(as.character(canopy))) %>%
  ...
  # Generate mean cover, season, and reporting period columns
  mutate(
    ...
    period = case_when(
      date >= as.Date(REP_START)  & date <= as.Date(REP_END) ~ "Reporting",
      date >= TREE_START          & date <= TREE_END         ~ "Baseline",
      TRUE ~ NA_character_)) %>%
  
  mutate(season = factor(season, levels = c(
    "Summer", "Autumn", "Winter", "Spring"))) %>%
  
  # Remove surveys before they became standardised
  filter(!is.na(period)) %>%
  ...
  # Join temperature
  left_join(weather_can, by = "date") 

# Export df to a new sheet in the running workbook
workbook <- export_sheet(canopy, sheet = "canopy")
```

### Statistics

```{r}
# Print statistics for canopy cover estimates
nrow(canopy)
min(canopy$date, na.rm = TRUE) # 2018-12-15
max(canopy$date, na.rm = TRUE) # 2024-09-29

# Calculate baseline means
(baseline_means <- canopy %>%
  ...
  summarise(
    ...
    min_dates  = paste(sort(unique(date[cover == min_cover])), 
                        collapse = ", "),

    max_cover  = max(cover, na.rm = TRUE),
    max_dates  = paste(sort(unique(date[cover == max_cover])), 
                       collapse = ", "),
    .groups    = "drop") %>%
  arrange(species, season))
```

> 🚩 *Draft until tree-by-tree data for 2025 reporting period is available*.

```{r, eval = FALSE}
# Calculate seasonal reporting-period statistics
(seasonal_reporting <- canopy %>%
  ...
  left_join(baseline_means, by = "species") %>%
  
  mutate(
    diff_abs = mean_cover - baseline_mean,
    diff_pct = 100 * diff_abs / baseline_mean) %>%
  
  arrange(species, season))
```

Here we fit per-species GLMs for canopy cover by flying-fox abundance, mean temperature, season, and year. 

```{r}
# Prepare canopy cover modelling data
canopy_mod <- canopy %>%
  filter(period == "Baseline") %>%
  mutate(
    date      = as.Date(date),
    ...
    ff        = as.numeric(ff)) %>%
  drop_na(cover, ff, mean_temp, season, year) %>%  
  droplevels()

# Fit GLM for canopy cover per species
glm_results <- canopy_mod %>%
  group_by(species) %>%
  nest() %>%
  ...
  mutate(
    model = map(
      data,
      ~ glm(
        cover ~ ff + mean_temp + season + year,
        data = .x,
        family = gaussian())),
  ...
  transmute(
    ...
    fixed_effect = case_when(
      term == "ff"                 ~ "Flying-fox abundance",
      term == "mean_temp"          ~ "Mean temperature",
      grepl("^season", term)       ~ "Season",
      grepl("^year", term)         ~ "Year",
      TRUE ~ term),
    p_value = p.value) %>%
  arrange(species, fixed_effect) %>%
...
(canopy_plot <- ggplot(
  ...
  scale_y_continuous(
    ...
    name     = "Estimated flying-fox abundance",
  ...
  theme(
    ...
    axis.text.y        = element_text(vjust  = 0.5),
    axis.title.x       = element_blank(),
    axis.title.y.left  = element_text(margin = ggplot2::margin(r = 12)),
    axis.title.y.right = element_text(margin = ggplot2::margin(l = 12)), 
    legend.position    = "bottom", 
    ...
    plot.margin        = margin(t = -0.35, r = 0.05, b = -0.2, l = 0.01, "cm"),
